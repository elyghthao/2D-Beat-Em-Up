...
public class Button_Press : MonoBehaviour
{
    ...
    private MeshRenderer button_mesh; // The button's mesh renderer.
    ...
    void Start()
    {
        // On start, the button's audio source, box collider, and mesh renderer
        // components are given to the associated variables for access, and
        // button_is_pressed is set to false.
        button_press_sound = GetComponent<AudioSource>();
        button_collider = GetComponent<BoxCollider>();
        button_mesh = GetComponent<MeshRenderer>();
